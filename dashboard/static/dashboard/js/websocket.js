...
socket.onmessage = function(event) {
  ...
  if (data.humidity !== undefined) {
    ...
    if (data.water_level !== undefined) {
      document.querySelector('.metric-card.water h2').innerHTML = `${data.water_level} <span>Pa</span>`;
    }
  ...
  if (data.water_level !== undefined) updateChart(charts.water, data.water_level);
